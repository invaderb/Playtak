...
function generateCamera(){
	...
	var cutleft=($('#game-details').hasClass('hidden')?0:209)+10
...
function adjustsidemenu(notation,chat){
	...
	if($('#game-details').hasClass('hidden')){
...
//  TODO rename functions to match sections
function rmenu() {
	if($('#game-details').hasClass('hidden')) {showrmenu()}
	else{hidermenu()}
}
...
function clearcolorchange(){
	...
	ccb.style.backgroundColor="#e2e2e2"
...
function setNetwork() {
	// get value
	let newHost = document.getElementById('#host').value;
	console.log(newHost)
...
$(function() {
	// set host
	localStorage.setItem('host', 'playtak.com')
